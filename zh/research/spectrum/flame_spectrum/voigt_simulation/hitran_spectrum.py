...
def main():
    ...
    """
    # 初始化HitranSpectrum类
    database_path = pathlib.Path('/home/xliu/prj/210-TDLAS/3-simulation/hitran_database')
    hitran = HitranSpectrum(database_path / '01_H2O/H2O-4650_5410.par', database_path / 'Q/')
    ...
    plt.subplot(2, 1, 1)
    plt.plot(wavenumber1, ab1, label='默认范围')
    plt.xlim(hitran.default_start, hitran.default_end)
    plt.legend()
    plt.ylabel('吸收率')
    plt.title('HITRAN光谱仿真 - 默认波数范围')
