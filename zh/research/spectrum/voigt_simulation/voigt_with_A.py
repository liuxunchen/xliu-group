...
def profile_voigt(nu,gamma_air,n_air,delta_air,T,p,wavenumber): # return voigt profile on wavenumber, function of (T,p), laser is the instrumetnal linewidth, gamma_air_ratio is the modification factor to the database value
...
#        S           = math.exp(line[1])
        ...
        #    peak = (profile*intensity*density)[int((nu - wavenumber[0])*1000)]
        #    if 10E5 > peak > 10E-5:
        #        ax1.bar(nu,peak,align='center',width=0.001,color='b')
...
path.append('/home/zgy/prj4um/3_hitran_simulation/voigt')
import voigt_with_database as voigt
...
hitran1 = np.array([
 ...
 [ 2397.077898, 4.357E-31, 2.162E+02, 0.0530, 0.059, 6606.8130, 0.69, -0.005192 ],#    0 0 0 11       0 0 0 01                    R130e     3377541927 5 4 5 7   259.0  257.0
    ])
...
database = hitran1[0:-1,:]
T=400
p = 0.01
c = 1
l = 500
# coef =  coef(T,p,database,wavenumber)
od,ab,tr = OD(T,p,c,l,database,wavenumber)

plt.plot(wavenumber,ab,label='hitemp') #/np.max(hitemp[(2397-start)/resolution:])
plt.xlim(start,end)
plt.legend(loc='best')
plt.show()
'''
