...
def profile_voigt(M,I,nu,gamma_air,n_air,delta_air,T,p,wavenumber): # return voigt profile on wavenumber, function of (T,p), laser is the instrumetnal linewidth, gamma_air_ratio is the modification factor to the database value
    ...
    variable = (wavenumber- nu - delta_air*p+gamma*1j)/(sigma*math.sqrt(2)) # air shift # xliu modify
    #np.savetxt('variable',variable)
    voigt = (special.wofz(variable)).real/(sigma*math.sqrt(2*constants.pi))

#    variable = (wavenumber- nu - delta_air*p+(gamma+p)*1j)/(sigma*math.sqrt(2)) 
#    variable = (wavenumber- nu - delta_air*p+(gamma+beta*p)*1j)/(sigma*math.sqrt(2)) 
...
def coef(T,p,database,loadfromdatabase,wavenumber): #color): # k 
    CDSD= np.array([
...
def OD(T,p,c,l,database,wavenumber): # optical density
    acoef = coef(T,p,database,False,wavenumber)*1E-11
...
def main():
    ...
    acoef = coef(T=2000,p=1,database='NA',loadfromdatabase=False,wavenumber=wn) # if loadfromdatabase = False, database='' can write any string does not matter

    # test wgq 20180501
    #wn = np.arange(2397.000,2397.402,0.001)  #test 20180501 check with wgq
    od,AB,Tr = OD(T=2000,p=1,c=1,l=0.1,database='database',wavenumber=wn) # optical density
